...
public class IamportService {
	...
	public IamportService(String apiKey, String apiSecret) {
		this.apiKey = apiKey;
		this.apiSecret = apiSecret;
		this.iamport = this.create(false);
	}

	public IamportService(String apiKey, String apiSecret, boolean useStaticIP) {
		this.apiKey = apiKey;
		this.apiSecret = apiSecret;
		this.iamport = this.create(useStaticIP);
	}

	public IamportService(String apiKey, String apiSecret, String tierCode) {
		this.apiKey = apiKey;
		this.apiSecret = apiSecret;
	...
	public IamportService(String apiKey, String apiSecret, String tierCode, boolean useStaticIP) {
		this.apiKey = apiKey;
		this.apiSecret = apiSecret;
		this.tierCode = tierCode;
		this.iamport = this.create(useStaticIP);
	...
	public IamportResponse postBillingCustomer(String customerUid, CardInfo card) throws IamportResponseException, IOException {
		AccessToken auth = getAuth().getResponse();
		Call<IamportResponse> call = this.iamport.post_billing_customer(auth.getToken(), customerUid, card);

		Response<IamportResponse> response = call.execute();
		if ( !response.isSuccessful() )	throw new IamportResponseException( getExceptionMessage(response), new HttpException(response) );

		return response.body();
	}

	public IamportResponse deleteBillingCustomer(String customerUid) throws IamportResponseException, IOException {
		AccessToken auth = getAuth().getResponse();
		Call<IamportResponse> call = this.iamport.delete_billing_customer(auth.getToken(), customerUid);
		...
		OkHttpClient client = new OkHttpClient.Builder()
				...
				.addInterceptor(new Interceptor() { //Tier Header
					public okhttp3.Response intercept(Chain chain) throws IOException {
						...
						if (IamportService.this.tierCode != null) {
							request = request.newBuilder().addHeader("Tier", IamportService.this.tierCode).build();
						}
